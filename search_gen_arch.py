...
def main():
    ...
    for ii in tqdm(range(args.Total_evolutionary_algebra), desc='search genearator using evo alg'):
        ...
        # 原有遗传算法流程
        population, pop_selected, is_mean, fid_mean, is_values, fid_values = trainer_gen.my_search_evolv2(population, fid_stat, ii)
        
        # 记录历史最优
        current_best_fitness = np.max(is_values) - np.min(fid_values)
        fitness_history.append((ii, is_mean, fid_mean, np.max(is_values), np.min(fid_values), current_best_fitness))
        
        if current_best_fitness > global_best_fitness:
            global_best_fitness = current_best_fitness
            print(f"全局最优更新: 代数={ii}, 适应度={current_best_fitness:.3f}")
        
        # 每5代保存历史记录
        if ii % 5 == 0:
            np.save(f'{args.path_helper["ckpt_path"]}/fitness_history.npy', np.array(fitness_history))
            
        # 早停机制
        if ii > 10:
            recent_improvements = [fitness_history[i][5] for i in range(max(0, ii-5), ii)]
            if len(set([round(x, 3) for x in recent_improvements])) == 1:  # 连续5代无改进
                print(f"连续5代无改进，在第{ii}代提前停止")
                break
