...
def merge_all_train_dataset():
    output = None
    for i in range(1, 11):
        filename = '/data/datasets/Imagenet32/npz/train_data_batch_' + str(i) + '.npz'
    ...
    np.savez('./fid_stat/ImageNet32_train_all.npz', output_array)
    print(f"{output_array.shape} size array saved into ImageNet32_train_all.npz")


def show_partial_samples():
    x = np.load('./fid_stat/ImageNet32_train_all.npz')['arr_0']
    img_arr = x[:16, :, :, :]
    np.savez('./fid_stat/ImageNet32_samples.npz', img_arr)
    print(f"{img_arr.shape} size array saved into ImageNet32_samples.npz")


def visualize_samples():
    x = np.load('./fid_stat/ImageNet32_samples.npz')['arr_0']
